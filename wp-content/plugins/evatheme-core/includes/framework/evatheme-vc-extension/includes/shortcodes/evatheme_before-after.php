...
if ( ! function_exists( 'cstheme_vc_before_after_shortcode' ) ) {
	function cstheme_vc_before_after_shortcode( $atts, $content = NULL ) {
		...
		wp_enqueue_script('despero_before_after', get_template_directory_uri() . '/assets/js/before-after.min.js', array('jquery'), false, true);
		wp_enqueue_style('despero_before_after', get_template_directory_uri() . '/assets/css/before_after.css');
        ...
        $compile = '<div class="evatheme-before-after-container">';
			$compile .= '<div class="evatheme-before-after">';
				$compile .= '<img src="' . esc_url( $image_1Src ) . '" title="' . esc_attr__( 'Before', 'evatheme_core' ) . '" />';
				$compile .= '<img src="' . esc_url( $image_2Src ) . '" title="' . esc_attr__( 'After', 'evatheme_core' ) . '" />';
			$compile .= '</div>';
		$compile .= '</div>';
...
if ( ! function_exists( 'cstheme_vc_before_after_shortcode_map' ) ) {
	function cstheme_vc_before_after_shortcode_map() {
		...
		vc_map(array(
			...
			'name'			=> esc_html__('Before / After', 'evatheme_core'),
			'description'	=> esc_html__('Display Before / After Images', 'evatheme_core'),
			...
			'params' 		=> array(
				array(
					'type' 			=> 'attach_image',
					'holder' 		=> 'img',
					'heading' 		=> esc_html__('Choose first image', 'evatheme_core'),
					'description' 	=> '',
					'param_name' 	=> 'image_1',
					'value' 		=> ''
				),
				array(
					'type' 			=> 'attach_image',
					'holder' 		=> 'img',
					'heading' 		=> esc_html__('Choose second image', 'evatheme_core'),
					'description' 	=> '',
					'param_name' 	=> 'image_2',
					'value' 		=> ''
				),
